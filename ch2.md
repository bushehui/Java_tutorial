...
- 关键字是Java事先定义的，有特别意义的标识符，有时又叫==保留字==。<br>
...
###Java 提供两种不同的类型：
__引用类型__（或__组合类型__）和__基本类型__（或__内置类型__）。

- Java的基本数据类型都有固定的数据位，不随运行平台的变化而变化。<br>
Java为每个基本类型提供了封装类。
...
float|实数|$4$|$-2^{31}$~$2^{31}$|$23.987$,$87F$
double|实数|$8$|$-2^{63}$~$2^{63}$|$2.0231$,$0.456D$
...
[^1]: Unicode字符集定义了一套国际标准字符集。通常的ASCII码是8位的，而Unicode字符集中的每个字符占16位，即2个字节，整个字符集共包括65536个字符，兼容ASCII，排在Unicode字符集最前面的256个字符就是ASCII码。Unicode除了可以表示256个ASCII码外，还可以表示汉字、拉丁语、希腊字母、朝鲜语等。
...
##常量
__常量__：程序运行过程中保持不变，并且不可进行修改的量。

Java语言中常量有两种类型：

- 直接常量也称为值常量，如12、-33、15.6；
- 符号常量，符号常量需要明确定义。
